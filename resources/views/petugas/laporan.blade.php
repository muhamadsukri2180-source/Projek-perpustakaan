...
<body class="bg-slate-50 text-slate-800 antialiased"
      x-data="{ activeTab: '{{ request()->routeIs('petugas.laporan.mingguan') ? 'mingguan' : (request()->routeIs('petugas.laporan.bulanan') ? 'bulanan' : (request()->routeIs('petugas.laporan.tahunan') ? 'tahunan' : 'harian')) }}' }">

    {{-- ==================== NAVBAR (disamakan dengan Dashboard Petugas) ==================== --}}
    ...
    <main class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
        ...
        <div class="bg-white rounded-2xl p-2 shadow-sm border border-slate-200/80 mb-6 flex overflow-x-auto gap-2">
            <button @click="activeTab = 'harian'" :class="activeTab === 'harian' ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'text-slate-600 hover:bg-slate-100'" class="px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 whitespace-nowrap">
                <i class="fa-solid fa-calendar-day"></i> Laporan Harian
            </button>
            <button @click="activeTab = 'mingguan'" :class="activeTab === 'mingguan' ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'text-slate-600 hover:bg-slate-100'" class="px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 whitespace-nowrap">
                <i class="fa-solid fa-calendar-week"></i> Laporan Mingguan
            </button>
            <button @click="activeTab = 'bulanan'" :class="activeTab === 'bulanan' ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'text-slate-600 hover:bg-slate-100'" class="px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 whitespace-nowrap">
                <i class="fa-solid fa-calendar-days"></i> Laporan Bulanan
            </button>
            <button @click="activeTab = 'tahunan'" :class="activeTab === 'tahunan' ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'text-slate-600 hover:bg-slate-100'" class="px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 whitespace-nowrap">
        ...
        <div class="space-y-6">
            ...
            <div x-show="activeTab === 'harian'" x-cloak class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <form action="{{ route('petugas.laporan.harian') }}" method="GET" class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 border-b border-slate-100 pb-4">
                    <div>
                        <h2 class="text-lg font-bold text-slate-800">Laporan Kunjungan Harian</h2>
                        <p class="text-xs text-slate-400">
                            {{ \Carbon\Carbon::parse($tanggal)->translatedFormat('l, d F Y') }}
                            &bull; Total: <span class="font-bold text-emerald-600">{{ $totalHarian }} Kunjungan</span>
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="date" name="tanggal" value="{{ $tanggal }}" class="px-3 py-2 rounded-xl border border-slate-200 text-xs outline-none focus:ring-2 focus:ring-emerald-500">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($laporanHarian as $index => $item)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="py-3 px-4 text-teal-600 font-medium">
            ...
            <div x-show="activeTab === 'mingguan'" x-cloak class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <form action="{{ route('petugas.laporan.mingguan') }}" method="GET" class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 border-b border-slate-100 pb-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-400">
                            ...
                            &bull; Total: <span class="font-bold text-emerald-600">{{ $totalMingguan }} Kunjungan</span>
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="week" name="minggu" value="{{ $awalMinggu->format('o-\WW') }}" class="px-3 py-2 rounded-xl border border-slate-200 text-xs outline-none focus:ring-2 focus:ring-emerald-500">
                        <button type="submit" class="bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-semibold hover:bg-emerald-700">Filter</button>
                    </div>
                </form>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($laporanMingguan as $row)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="py-3 px-4 text-center font-bold text-emerald-600">{{ $row['total'] }} Siswa</td>
                                </tr>
                            @empty
                                <tr>
            ...
            <div x-show="activeTab === 'bulanan'" x-cloak class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <form action="{{ route('petugas.laporan.bulanan') }}" method="GET" class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 border-b border-slate-100 pb-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-400">
                            ...
                            &bull; Total: <span class="font-bold text-emerald-600">{{ $totalBulanan }} Kunjungan</span>
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="month" name="bulan" value="{{ $bulanInput }}" class="px-3 py-2 rounded-xl border border-slate-200 text-xs outline-none focus:ring-2 focus:ring-emerald-500">
                        <button type="submit" class="bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-semibold hover:bg-emerald-700">Filter</button>
                    </div>
                </form>
            ...
            <div x-show="activeTab === 'tahunan'" x-cloak class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <form action="{{ route('petugas.laporan.tahunan') }}" method="GET" class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 border-b border-slate-100 pb-4">
                    <div>
                        <h2 class="text-lg font-bold text-slate-800">Laporan Kunjungan Tahunan</h2>
                        <p class="text-xs text-slate-400">
                            Tahun {{ $tahun }}
                            &bull; Total: <span class="font-bold text-emerald-600">{{ $totalTahunan }} Kunjungan</span>
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        <select name="tahun" class="px-3 py-2 rounded-xl border border-slate-200 text-xs outline-none focus:ring-2 focus:ring-emerald-500">
